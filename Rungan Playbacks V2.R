...
# V2 Calculate signal and noise separately
...
# Assign number of seconds on each side of selection for noise
noise.seconds <- 10
...
for( a in 1:length(selection.tables)){
  ...
  for(b in 1: (length(playback.breaks)-1)){
    ...
    # Add specified seconds of noise
    for(c in 1:nrow(single.playback.selections)){
      ...
      short.wav <- cutw(temp.wav,from = (temp.wav.cut$Begin.Time..s.-noise.seconds),to= (temp.wav.cut$End.Time..s.+noise.seconds),output = 'Wave')
      ...
      # Calculate noise for each 1 sec bin for the 60-s recording
      noise.list <- list()
      ...
      # Code to calculate signal
      # Cut wave file to focus on selection only
      short.wav.no.noise <- cutw(temp.wav,from = (temp.wav.cut$Begin.Time..s.),
                                 to= (temp.wav.cut$End.Time..s.),output = 'Wave')
      
      # Filter to frequency of interest
      w.dn.filt.no.noise <- fir(short.wav.no.noise, from=temp.wav.cut$Low.Freq..Hz., to=temp.wav.cut$High.Freq..Hz.,output = "Wave") 
      w.dn.filt.no.noise <- tuneR::normalize(w.dn.filt.no.noise, unit="16")
      
      # NOTE: The commented out section needs to be fixed so that the SNR values are consistent with what we did previously
      # # Create 10-ms bins and calculate sum of squares for each
      # bin.seq.smaller <- seq(from=0, to=length(short.wav.no.noise@left), by=160)
      # bin.seq.length.small <- length(bin.seq.smaller)-1
      # 
      # # Output list of shorter wav file clips
      # subsamps.short <- lapply(1:bin.seq.length.small, function(i) 
      #   extractWave(short.wav.no.noise, from=as.numeric(bin.seq.smaller[i]), 
      #               to=as.numeric(bin.seq.smaller[i+1]), xunit = c("samples"),
      #               plot=F,output="Wave"))
      # 
      # # Calculate sum of squares for each clip
      # sum.square.list <- list ()
      # for (f in 1:length(subsamps.short)){
      #   wav.temp <- subsamps.short[[f]]
      #   sum.square.list[[f]] <- sum(wav.temp@left^2)
      ...
      #Find 99% quantile value for all 10 ms clips
      # signal.value <- quantile(unlist(sum.square.list), c(.99))
      signal.value <- sum(short.wav.no.noise@left^2)
      
      # Calculate SNR in dB
      # Here we are using 99% quantile value
      SNR.dB.high <- 10 * log10((signal.value-noise.value)/noise.value)
      print(SNR.dB.high)
